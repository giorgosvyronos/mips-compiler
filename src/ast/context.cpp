...
string Context::getFreeRegister(ostream& fileOut,const string& parameter_flag){
  uint32_t index = -1;
  ...
  if ((index>=16)&&(index<=23)){
    ...
    if (registers[index].getFP().compare("4294967295")==0){
      ...
      fileOut<<"\t"<<"sw "<<reg_id<<","<<frame_pointer<<"($fp)"
  ...
  frame_pointer+=4;
  saveVariable(fileOut,var_id,false);
...
bool Context::isInContext(ostream& fileOut, const string& var_id){
  ...
  if (it != bindings.end()){
    ...
    if(var_name.compare("")!=0){
      string reg_id = getFreeRegister(fileOut,"");
      loadVariable(fileOut, var_id, reg_id,true);
...
void Context::saveVariable(ostream& fileOut, const string& var_id, const bool& erase){
  for (size_t i = 0; i < variables.size(); i++) {
    if ((variables[i].getVarId().compare(var_id)==0)&&(variables[i].getFP(false).compare("4294967295")!=0)
        ...
        fileOut<<"\t"<<"sw "<<bindings[variables[i].getVarId()]<<","<<frame_pointer<<"($fp)"
    ...
    if ((variables[i].getVarId().compare(var_id)==0)&&(variables[i].getType().compare("global")==0)){
      ...
      fileOut<<"\t"<<"lui $t9,"<<"%hi("<<variables[i].getVarId()<<")"<<endl;
      fileOut<<"\t"<<"sw "<<bindings[variables[i].getVarId()]
...
void Context::loadVariable(ostream& fileOut, const string& var_id, const string& reg_id, const bool& erase){
  for (size_t i = 0; i < variables.size(); i++) {
    if ((variables[i].getVarId().compare(var_id)==0)&&(variables[i].getFP(false).compare("4294967295")!=0)
        &&(variables[i].getType().compare("scratch")!=0)&&(variables[i].getType().compare("array")!=0)){
        string frame_pointer = variables[i].getFP(erase);
        fileOut<<"\t"<<"lw "<<reg_id<<","<<frame_pointer<<"($fp)"
        <<"\t #Loading variable: "<<variables[i].getVarId()<<endl;
        continue;
    }
    if ((variables[i].getVarId().compare(var_id)==0)&&(variables[i].getType().compare("global")==0)){
      fileOut<<"\t"<<"move $t9,$0"<<endl;
      fileOut<<"\t"<<"lui $t9,"<<"%hi("<<var_id<<")"<<endl;
      fileOut<<"\t"<<"lw "<<reg_id<<","<<"%lo("<<var_id<<")"<<"($t9)"<<endl;
      fileOut<<"\t"<<"move $t9,$0"<<endl;
    }
  }
}
...
void Context::makeBinding(ostream& fileOut, const string& var_id, const string& reg_id, const string& type){
    bindings[var_id] = reg_id;
    for (size_t i = 0; i <= 31; i++) {
      if (registers[i].getRegId().compare(reg_id)==0){
        registers[i].setVarId(var_id);
...
void Context::updateBinding(ostream& fileOut, const string& var_id, const string& type){
...
void Context::allocateStack(ostream& fileOut, const int32_t& val){
  //Allocate stack
  fileOut<<"\t"<<"addiu $sp,$sp,"<<to_string(-1*(val + stack_pointer))<<endl;
  fileOut<<"\t"<<"sw $31,"<<to_string(val+ stack_pointer-4)<<"($sp)"
  <<"\t"<<"# Storing return address"<<endl;
  fileOut<<"\t"<<"sw $fp,"<<to_string(val+ stack_pointer-8)<<"($sp)"
  <<"\t"<<"# Storing current frame pointer"<<endl;
  fileOut<<"\t"<<"move $fp,$sp"<<endl;
  stack_pointer+= val;
  //Reset the frame pointer
  frame_pointer=0;

}
void Context::deallocateStack(ostream& fileOut,const int32_t& val){
  fileOut<<"\t"<<"move $sp,$fp"<<endl;
  fileOut<<"\t"<<"lw $fp,"<<to_string(stack_pointer-8)<<"($sp)"
  <<"\t"<<"# Loading current frame pointer"<<endl;
  fileOut<<"\t"<<"lw $31,"<<to_string(stack_pointer-4)<<"($sp)"
  <<"\t"<<"# Loading return address"<<endl;
  fileOut<<"\t"<<"addiu $sp,$sp,"<<to_string(stack_pointer)<<endl;
...
void Context::clearScopeContext(ostream& fileOut){
  ...
  for (it = bindings.begin(); it != bindings.end(); it++){
    ...
    for (size_t i = 0; i<variables.size(); i++){
      ...
      && (variables[i].getFPStackSize()==1)){
        ...
        fileOut<<"\t #Releasing binding: "<<var_id<<endl;
      ...
      &&  (variables[i].getRegId().compare("")!=0)){
        ...
        loadVariable(fileOut,variables[i].getVarId(), bindings[variables[i].getVarId()], true);
        variables[i].getSN(true);
      }
      if ((variables[i].getSN(false)>scope_counter)){
        fileOut<<"\t # Decrementing SN, FP"<<endl;
        variables[i].getSN(true);
        variables[i].getFP(true);
  ...
  fileOut<<"\t #==Clearing scope context=="<<endl;
}

void Context::saveCurrentVariables(ostream& fileOut){
  fileOut<<"\t #==Saving current variables=="<<endl;
  for (size_t i = 4; i<=30; i++){
    string reg_id = registers[i].getRegId();
    if (registers[i].getVarId()!=""){
      fileOut<<"\t"<<"sw "<<reg_id<<","<<to_string(frame_pointer)<<"($fp)"<<endl;
      frame_pointer+=4;
    }
  }
}
void Context::loadCurrentVariables(ostream& fileOut){
  fileOut<<"\t #==Loading current variables=="<<endl;
  for (size_t i = 30; i>=4; --i){
    string reg_id = registers[i].getRegId();
    if (registers[i].getVarId()!=""){
      frame_pointer-=4;
      fileOut<<"\t"<<"lw "<<reg_id<<","<<to_string(frame_pointer)<<"($fp)"<<endl;
    }
  }
}
void Context::loadLocalVariables(ostream& fileOut){
  fileOut<<"\t #==Loading used variables 16-23=="<<endl;
  for (size_t i=16; i<=23; i++){
    if (registers[i].getFP().compare("4294967295")!=0){
      string reg_id = registers[i].getRegId();
      string r_frame_pointer = registers[i].getFP();
      fileOut<<"\t"<<"lw "<<reg_id<<","<<r_frame_pointer<<"($fp)"<<endl;
      registers[i].setFP(-1);
    }
  }
}
...
uint32_t Context::getIfCounter(){                           return if_counter;  }
void Context::incrementIfCounter(){                         if_counter++; }
void Context::decrementIfCounter(){                         if_counter--; }
...
uint32_t Context::getEnumCounter(){                         return enum_counter;}
void Context::incrementEnumCounter(){                       enum_counter++; }
void Context::decrementEnumCounter(){                       enum_counter--; }
